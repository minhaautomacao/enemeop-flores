...
// O Meta não manda nome do remetente em eventos de mensagem (só em
// comentários), então nome_cliente frequentemente fica nulo mesmo numa
// conversa real — nunca mostra o ID técnico como nome principal, cai num
// rótulo genérico por canal; o ID continua disponível como diagnóstico.
function nomeExibido(c: { nome_cliente: string | null; canal: string }): string {
  return c.nome_cliente || (c.canal === 'facebook' ? 'Cliente do Facebook' : 'Cliente do Instagram');
}
    ...
    <div className="p-6 space-y-4">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[380px_1fr] gap-4 min-h-[680px]">
        <aside className="card p-0 overflow-hidden"><div className="px-4 py-3 border-b border-border flex justify-between"><b className="text-sm">Fila</b><span className="text-xs text-text-faint">{conversas.length}</span></div><div className="divide-y divide-border max-h-[640px] overflow-y-auto">
          {conversas.map(c => { const status = c.status_atendimento ?? 'flora_atendendo'; const alerta = ['aguardando_humano','erro_envio'].includes(status); return <button key={c.id} onClick={() => setSelecionada(c.id)} className={`w-full text-left p-4 hover:bg-surface-hover ${c.id === selecionada ? 'bg-gold/10' : ''}`}><div className="flex items-center justify-between gap-2"><b className="text-sm text-text-primary truncate">{nomeExibido(c)}</b>{alerta && <AlertTriangle className="w-4 h-4 text-status-warning"/>}</div><p className="text-xs text-text-faint mt-1">{CANAL[c.canal] ?? c.canal} • {hora(c.atualizado_em)}</p><p className="text-xs text-text-muted truncate mt-2">{ultima(c)}</p><div className="flex justify-between mt-3"><span className={`text-[10px] px-2 py-0.5 rounded-full ${alerta ? 'badge-warning' : 'badge-info'}`}>{STATUS[status] ?? status}</span><span className="text-[10px] text-text-faint">{espera(c.handoff_em ?? c.atualizado_em)}</span></div></button>; })}
        </div></aside>
