...
export async function POST(_req: Request, { params }: { params: { id: string } }) {
  ...
  if (error || !data) return NextResponse.json({ error: 'Somente o atendente responsável pode devolver esta conversa' }, { status: 403 });

  await (supabase as any).from('atendimentos_humanos')
    .update({ status: 'devolvido_flora', devolvido_em: new Date().toISOString(), atualizado_em: new Date().toISOString() } as any)
