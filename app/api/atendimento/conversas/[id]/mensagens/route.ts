...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  ...
  if (error || !conversa) return NextResponse.json({ error: 'Conversa n?o encontrada' }, { status: 404 });
  if ((conversa as any).status_atendimento === 'concluida') return NextResponse.json({ error: 'Conversa conclu?da' }, { status: 409 });
  ...
  const factoryUrl = process.env.NEXT_PUBLIC_FABRICA_URL ?? 'https://ebeapnydeiwuewxatuuw.supabase.co';
  const secret = process.env.FLORA_INBOX_SECRET ?? process.env.FACTORY_SECRET ?? '';
  if (!secret) return NextResponse.json({ error: 'FLORA_INBOX_SECRET n?o configurado no servidor' }, { status: 500 });
  const edge = await fetch(`${factoryUrl}/functions/v1/webhook-meta`, {
    method: 'POST',
